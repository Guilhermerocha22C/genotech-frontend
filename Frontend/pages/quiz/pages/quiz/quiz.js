...
async function finalizar() {
    console.log("Salvando acertos:", pontos);
    localStorage.setItem("acertos", pontos.toString());
    
    const userId = localStorage.getItem('userId');
    if (userId) {
        console.log("userId já está no localStorage:", userId);
    } else {
...
// Novo código para o popup do sistema de pontos
function mostrarSistemaPontos() {
    const sistemaPontosPopup = document.getElementById('sistema-pontos-popup');
    sistemaPontosPopup.style.display = 'block';
}

function esconderSistemaPontos() {
    const sistemaPontosPopup = document.getElementById('sistema-pontos-popup');
    sistemaPontosPopup.style.display = 'none';
}

function configurarPopupSistemaPontos() {
    const sistemaPontosPopup = document.getElementById('sistema-pontos-popup');
    const fecharSistemaPontos = document.getElementById('fechar-sistema-pontos');
    
    mostrarSistemaPontos();

    fecharSistemaPontos.addEventListener('click', function() {
        esconderSistemaPontos();
        iniciarQuiz();
    });

    window.addEventListener('click', function(event) {
        if (event.target === sistemaPontosPopup) {
            esconderSistemaPontos();
        }
    });
}
