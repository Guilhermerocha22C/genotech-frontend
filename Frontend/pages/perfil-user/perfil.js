...
async function updateProfile() {
    ...
    try {
        const response = await fetch(`https://genotechfullstack.vercel.app/api/user/update/${userData.id}`, {
            method: 'PUT',
            body: formData
        });
        ...
        if (data.success) {
            ...
            if (data.imageUrl) {
                userData.profileImageUrl = `https://genotechfullstack.vercel.app${data.imageUrl}`;
            }
...
async function loadUserData() {
    ...
    try {
        const response = await fetch(`https://genotechfullstack.vercel.app/api/user/${user.id}`);
        const data = await response.json();
